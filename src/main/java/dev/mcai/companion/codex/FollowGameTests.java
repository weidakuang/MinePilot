...
public final class FollowGameTests {
    ...
    private static final class Gate {
        ...
        final String pace=System.getenv().getOrDefault("MINEPILOT_FOLLOW_TEST_PACE","walk");
        HeadlessPlayerSession human;String request;int phase;long began;Vec3 held;Vec3 humanStart;Vec3 restingPosition;
        boolean replaced,turnedWhileWalking,sawRestJump;int restCycles;long restingSince=-1;int movingTicks,stoppedWhileTargetMoved;long resumed=-1;final JsonArray evidence=new JsonArray();
        ...
        void start(){
            h.assertTrue(java.util.Set.of("walk","sprint","auto").contains(pace),"Unsupported test pace");
            runtime.server().setDifficulty(net.minecraft.world.Difficulty.PEACEFUL,true);
            var origin=h.absolutePos(new BlockPos(4,2,20));
            for(int x=-2;x<=38;x++)for(int z=-8;z<=18;z++){
            ...
            h.addCleanup(ignored->{saveEvidence();human.close();runtime.onChat("TestHuman","停下");});
            begin();h.onEachTick(this::tick);
        }
        void begin(){
            var accepted=call("request_navigation","{\"target_kind\":\"player\",\"target_name\":\"FollowHuman\",\"acceptance_radius\":2,\"preferred_pace\":\""+pace+"\",\"continuous_follow\":true,\"player_intent\":\"keep following\"}");
        ...
        void startWalk(){
            held=runtime.player().position();humanStart=human.player().position();began=h.getTick();resumed=-1;
            movingTicks=0;stoppedWhileTargetMoved=0;phase=2;
            human.player().applyControlFrame(new AgentControlFrame(restCycles==2?0:-90,0,restCycles==0?1:restCycles==1?.4F:.6F,0,false,false,false));
        }
        void tick(){
            ...
            if(phase==1 && state.equals("FOLLOWING") && !replaced){
                ...
                for(String target: new String[]{"MissingFollowTarget", "FollowHuman"}) {
                    ...
                    args.addProperty("preferred_pace",pace);args.addProperty("player_intent","replace goal");
                ...
                var args=new JsonObject();args.addProperty("target_kind","player");args.addProperty("target_name","FollowHuman");args.addProperty("preferred_pace",pace);args.addProperty("player_intent","replace goal");args.addProperty("replace_request_id",old);args.addProperty("continuous_follow",true);
            ...
            if(phase==2){
                if(restCycles==2 && !turnedWhileWalking && human.player().position().distanceTo(humanStart)>=5) {
                    turnedWhileWalking=true;
                    human.player().applyControlFrame(new AgentControlFrame(-90,0,.6F,0,false,false,false));
                }
                if(p.tickCount%10==0)h.assertTrue(Math.abs(p.getXRot())<=25,"Walking gaze points steeply at the feet: "+p.getXRot());
                if(resumed>=0 && human.player().position().distanceTo(humanStart)>4) {
                ...
                if(human.player().position().distanceTo(humanStart)>=10){human.player().stopControlling();phase=3;}
                return;
            }
            if(phase==3 && state.equals("FOLLOWING")){
                if(restCycles==0 && human.player().getY()-p.getY()>.5)sawRestJump=true;
                ...
                if(restCycles<2) {
                    if(restingSince<0){restingSince=h.getTick();restingPosition=p.position();}
                    if(restCycles==0 && h.getTick()-restingSince==10)
                        human.player().applyControlFrame(new AgentControlFrame(-90,0,0,0,true,false,false));
                    if(restCycles==0 && h.getTick()-restingSince==11)human.player().stopControlling();
                    h.assertTrue(p.position().distanceTo(restingPosition)<.05,"Resting follow moved toward an obsolete waypoint when the nearby player jumped");
                    if(h.getTick()-restingSince<80)return;
                    if(restCycles==0)h.assertTrue(sawRestJump,"Nearby player never physically jumped during the resting follow check");
                    h.assertTrue(status.get("requestId").getAsString().equals(request),"Rest discarded the follow subscription");
                    dev.mcai.companion.MinecraftAiCompanion.LOGGER.info("Follow leg {}: resume={} ticks, stopped={}/{} moving ticks",restCycles,resumed,stoppedWhileTargetMoved,movingTicks);
                    restCycles++;restingSince=-1;startWalk();return;
                }
                call("cancel_navigation","{\"request_id\":\""+request+"\",\"reason\":\"test stop\"}");
                dev.mcai.companion.MinecraftAiCompanion.LOGGER.info("Follow turning leg: resume={} ticks, stopped={}/{} moving ticks",resumed,stoppedWhileTargetMoved,movingTicks);
